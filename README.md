...
## Multi-tenant setup (Option B) — new customer onboarding, ready to use

The documented, semi-automated approach we agreed on: giving a new jeweller their own completely separate Firebase project (own login, own database, own photo storage — nothing shared with Shah Jewellers or any other customer), without building a full automated backend panel for it.

- **`NEW-CUSTOMER-SETUP.md`** — the complete, plain-language checklist, start to finish. Covers the one-time prerequisites (installing the Firebase CLI, logging in — done once ever, not per customer), the per-customer steps, and a troubleshooting section for the errors most likely to actually come up.
- **`scripts/setup-new-tenant.sh`** — automates the tedious, easy-to-get-wrong part: creating the new project, deploying the exact security rules and indexes this app needs, registering a web app, and printing the connection keys needed for `firebase-config.js`. Run as `./scripts/setup-new-tenant.sh <project-id> "<Shop Name>"`.
- **`firestore.indexes.json`** — codifies every index this app actually needs (verified against everything set up throughout this whole build: the Timeline composite index, and the three collection-group indexes for Reports and the surplus-funds Dashboard widget), so a new project gets them all in one deploy command instead of manually re-clicking through Firestore's "create index" links one at a time, the way we originally had to for Shah Jewellers.
- **`firebase.json`** — ties the rules and indexes files together so the Firebase CLI knows what to deploy.

**Honest about what's still manual, not oversold as fully automated:** creating a brand-new Firebase project doesn't come with Firestore, Authentication, or Storage switched on by default — each needs enabling once, by hand, in the Firebase console, before the automated deploy step will succeed. The script pauses at exactly that point and tells you so, rather than failing confusingly partway through. Deploying to Vercel as a separate project and creating the first login are also manual — genuinely quick, but not things the Firebase CLI can do on its own.

**Verified the index definitions carefully, not guessed** — looked up Firestore's actual documented syntax for collection-group single-field indexes (a different mechanism than regular composite indexes, since these were originally set up as "exemptions" via the console) before writing `firestore.indexes.json`, to make sure the script would actually reproduce what's already working, not something subtly different.

New files: `firebase.json`, `firestore.indexes.json`, `NEW-CUSTOMER-SETUP.md`, `scripts/setup-new-tenant.sh`.

## Found the actual cause of the Loans page crash — a real bug in how translation was applied

You found the exact error in your browser console: `Cannot set properties of null (setting 'textContent') at render (loans.js:213:61)`. This pointed straight at the real cause, which turned out to be a genuine bug introduced by the translation work itself, not the two defensive fixes from before (those were real improvements too, but not this specific crash).

**What was actually happening:** the "⚙ Filters" button on the Loans page has a small nested counter badge inside it (`<span id="filtersActiveCount">`) that shows how many filters are active. When the button itself got a `data-i18n` translation attribute, `applyTranslations()` — which sets `.textContent` on any element it translates — **completely wiped out that nested badge**, since setting `.textContent` on an element destroys all of its children, not just its own text. The badge span was gone from the page entirely. Later, when `render()` tried to update that badge's count, `document.getElementById("filtersActiveCount")` returned `null` — because the element no longer existed — and the whole page crashed right there, before it ever got the chance to display anything. That's exactly why it looked like it was stuck "Loading…" forever.

**Fixed at the actual root, not just for this one button**: `applyTranslations()` now only replaces an element's own direct text, and explicitly leaves any nested child elements completely untouched — the safe behavior for both the simple case (plain text, works exactly as before) and this nested case (badge survives). Then audited every one of the 325 translated keys across all 7 pages for this exact same pattern, twice — once specifically for nested elements with an `id` (the kind that would cause a crash like this one), and once more broadly for *any* nested element at all (which would've been a silent visual bug, even without a crash). **This Filters button was the only instance anywhere in the app** — confirmed, not assumed.

Touches: `js/i18n.js` (the actual fix).

## Fixed: Loans page could get stuck on "Loading…" forever

Found two real, related gaps while investigating this — both are genuine robustness issues, not cosmetic.

**1. No error handling on the customer lookup that runs before anything else loads.** If that one query failed for any reason — even a brief network hiccup — it threw an *uncaught* error that happened before the loans query or the page render ever ran. The result: the page's initial "Loading…" placeholder just stays there forever, with the real cause only visible in the browser console (easy to miss, especially on a phone). Fixed with proper error handling — a failure here now shows a clear toast and the page still loads (village/mobile columns may just be temporarily blank).

**2. One bad loan could silently block every other loan in the same status from showing at all.** The batch of loans for a status tab was all-or-nothing — if even a single loan had a data issue that made the interest calculation throw (e.g. corrupted or missing data on one old record), *none* of the loans for that status would appear, even though the rest were completely fine. Fixed to handle each loan independently: a broken one is now skipped with a clear console message and a toast telling you how many were skipped, while every other loan still loads and displays normally.

Verified the fix directly: simulated a mix of working and broken loan records, confirmed the working ones correctly show while the broken one is cleanly skipped rather than taking down the whole list.

**If this happens again, the browser console (F12) will now show a specific, real error message** rather than the page just silently hanging — that's the fastest way to pin down the exact cause if it recurs.

Touches: `js/loans.js` only.

## App translation — complete across all 7 pages (static UI)

**Customer Profile is done — the last page.** 34 more keys added, bringing the final total to **325 keys across all 4 languages**, covering every page in the app: Dashboard, Customers, Loans, New Loan, Reports, Loan Detail, and Customer Profile.

**Found and fixed a real, significant bug at the very start of this work, before adding anything new**: `applyTranslations()` — the function that actually applies `data-i18n` translations to a page — was defined but **never actually called anywhere except the login page**. Nav labels worked because they're built directly with `t()` in JavaScript, not through this mechanism, which is why the gap wasn't obvious. This meant any `data-i18n` attribute added to any other page would have silently done nothing at all. Fixed by calling it from `renderShell()`, which every authenticated page already invokes — one fix, applies everywhere, confirmed working on every page added since.

**Also caught and fixed a real, separate bug while going through Loan Detail**: the "Interest breakdown" section still said "a minimum of 1 month's interest always applies" — leftover text from before that rule was completely removed several turns ago, factually wrong ever since. Fixed to correctly describe the current pure day-wise method.

**Verified thoroughly, not just assumed correct**: every single addition was checked with an exact key-parity comparison (every language's key set matched against English — zero missing, zero mistyped, across all 325 keys) plus a final cross-reference confirming every `data-i18n` attribute actually used anywhere in the HTML across all 7 pages resolves to a real translation key. Both checks pass clean.

**What this covers, precisely**: static UI chrome — headings, labels, buttons, placeholders, hints, table headers, dropdown/filter options. **What it deliberately doesn't cover**: text generated dynamically inside JavaScript — table rows built from live data, toast notifications, error messages. Translating those would require changing how each page renders its content internally, not just adding attributes to static HTML — a meaningfully different, larger undertaking if wanted later.

Touches (this final page): `pages/customer-profile.html`, `js/i18n.js`.
