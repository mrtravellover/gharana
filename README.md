...
- **Customer surplus funds** — money a customer has left with you for safekeeping, tracked separately from their loans (this is money *you* owe *them*, the reverse of a loan). Lives on each customer's profile page under "💰 Surplus funds held." Interest is optional per deposit, since you said it varies — when it applies, it reuses the exact same 365/366-day interest engine as loans, just calculating what's owed to the customer instead of by them. Three states: **held** → **requested** (customer says they need it back — this is the important one) → **returned** (locks in the final payable amount). The moment something is marked "requested," it also appears on the **Dashboard's new "💰 Funds to return" section**, right next to "Ready to release" — so if a customer has both a loan ready for release *and* a requested deposit, you see both together and don't forget to bring the cash along with the ornaments. **One-time setup required:** this dashboard section uses a Firestore *collection-group* query (to find every customer's requested deposits in one call, without looping through customers individually) — Firestore requires a manual index for this the first time it's used. If the section doesn't appear after you mark something as "requested," open your browser's developer console; Firestore's error message includes a direct link that creates the needed index with one click. After that first time, it works automatically going forward.
