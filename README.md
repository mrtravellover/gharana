...
## One-time minimum interest period (new) — replaces the old "always floor" rule

The original rule was: a 1-month minimum charge applies every time any interest period is calculated, however short. Through direct testing you found this was wrong in a specific, important way — after a loan's interest was fully cleared, the *next* time it was checked, the system was charging a brand new month's minimum again, rather than continuing with exact day-wise interest. The corrected rule, locked down precisely before any code changed:

- **The 1-month minimum applies exactly once per qualifying disbursement** — a fixed calendar window from the disbursement date to the same date next month (e.g. 3-Jun → 3-Jul).
- **Any payment before that window closes must cover the full minimum** — however many payments it takes to reach that fixed amount, nothing further is owed until the window's end date, no matter how many times the customer comes back in between.
- **Once the window closes — permanently, forever after — interest is pure exact day-wise, no floor ever again**, even if the customer later clears the balance to zero multiple times.
- **Only two kinds of disbursement get this window**: a loan's very first disbursement, and a new disbursement added while reactivating a loan that had reached "released" status (treated as a fresh start, your explicit call). A simple top-up added to a loan that's still active gets pure day-wise interest from day one — never a minimum, ever.
- **Verified with 9 targeted tests** covering every scenario discussed: paying the minimum in one go vs. several partial payments, checking the balance multiple times mid-window, the transition the moment the window closes, a top-up on an active loan, a reactivation disbursement, backward compatibility for loans created before this field existed, and — critically — that this doesn't reintroduce or interact badly with the rounding-spillover bug fixed just before this. All passed.

Every disbursement now carries a `startsMinimumPeriod` flag, set at creation time — always `true` for a loan's first disbursement, and set based on whether the loan's status was "released" at the moment for any later disbursement. Loans created before this flag existed fall back to "only the earliest disbursement qualifies" — the correct behavior for the very common single-disbursement case, and a reasonable default for older data.

Touches: `js/interest.js` (the core rule), `js/loan-create.js`, `js/loan-detail.js` (setting the flag correctly at both loan creation and later disbursements).
