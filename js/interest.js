...
// Partial-month rule: only the loan's FIRST disbursement rounds up any part
// of a month to a full month (minimum 1 month, classic sarafa practice).
// Every disbursement after that is charged for the exact days held instead —
// no monthly rounding, no minimum. ROUND_MODE below only affects that first
// disbursement; change it to "exact" to remove the minimum entirely.
// ============================================================

const ROUND_MODE = "roundup"; // "roundup" | "exact" — applies to the first disbursement only
...
// Real elapsed duration as "9 months, 22 days" — for display only.
// This is the ACTUAL time passed; the months charged (for the minimum-month
// billing rule) is a separate, rounded-up number shown alongside it.
 ...
 * months), "compound" (compounds every month: Principal × ((1+rate%)^months − 1)),
 * or "compound_annual" (simple monthly interest accrues, but only folds into
 * the principal — compounds — once every 12 months).
 ...
 *
 * Minimum charge: only the loan's very first (oldest) disbursement gets the
 * 1-month minimum — any part-month started on it still counts as a full
 * month, so it kept for 1 day still owes 1 month's interest. Every other
 * disbursement (a top-up, a re-lend on a released mortgage, etc.) is charged
 * for the exact days it was actually held — no monthly rounding, no minimum.
 * The per-disbursement breakdown shows the exact day count alongside what's
 * actually charged, so it's easy to verify by hand.
 */
function periodInterest(principal, ratePct, months, type) {
  ...
  if (type === "compound_annual") {
    // Simple interest accrues monthly, but is folded into the principal
    // (compounds) only once every 12 months — not every month.
    let p = principal;
    ...
    p += p * (ratePct / 100) * remaining;
    return round2(p - principal);
  }
...
function calcLoanSummary(disbursements, payments = [], asOfDate = new Date()) {
  ...
  // Rule: only the loan's very first (oldest) disbursement gets the 1-month
  // minimum. Every disbursement after that — a top-up, a re-lend on a
  // released mortgage, etc. — is charged for the exact days it was actually
  // held, with no rounding up to a full month.
  ...
  for (const ev of events) {
    if (ev.kind === "ratechange") {
      ...
      const months = monthsElapsed(s.periodStart, ev.date, s.roundMode);
  ...
  const perDisbursement = states.map((s) => {
    ...
    const months = monthsElapsed(s.periodStart, asOfDate, s.roundMode);
