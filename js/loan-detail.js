...
async function handleGenerateReceipt(action) {
  ...
  const btn = document.getElementById(action === "print" ? "receiptPrintBtn" : "receiptWhatsappBtn");
  if (btn) { btn.disabled = true; btn.textContent = "Generating…"; }
  const receiptNumber = await generateReceipt(type, ctx);
  if (btn) { btn.disabled = false; btn.textContent = action === "print" ? "🖨 Print / Save PDF" : "💬 Share WhatsApp"; }
