...
async function saveOrnamentEdit() {
  ...
  await logActivity(loanId, "Item edited", `${changes.join(", ") || "no field changes"}${reason ? ` — ${reason}` : ""}`);
...
async function saveDisbursementEdit() {
  ...
  await logActivity(loanId, "Disbursement edited", `${changes.join(", ") || "no amount/date change"}${note ? ` — ${note}` : ""}`);
