  ...
  document.getElementById("loanHeader").innerHTML = `
    <div class="section-head">
      <div>
        ...
        <p style="font-size:13px;color:var(--ink-soft);margin-top:6px;cursor:pointer;" onclick="openValuationLimitModal()" title="Click to edit">
          Valuation limit: ${loanData.valuationLimit ? `<strong style="color:var(--ink);">${fmtMoney(loanData.valuationLimit)}</strong>` : "not set"} <span style="text-decoration:underline;">✏️ edit</span>
        </p>
...
function renderOrnaments() {
  ...
  // Highlighted weight summary — only shows the metals/categories actually
  // present, so a gold-only loan doesn't show a meaningless "Haali: 0g".
  let haaliWt = 0, paatWt = 0, goldWt = 0;
  ornaments.forEach((o) => {
    const wt = (Number(o.weight) || 0) * (Number(o.qty) || 1);
    if (o.metalType === "Gold") { goldWt += wt; }
    else if (o.category === "Haali") { haaliWt += wt; }
    else if (o.category === "Paat") { paatWt += wt; }
  });
  const silverTotal = haaliWt + paatWt;
  const chip = (label, value, color) => `<div style="background:${color}22;color:${color};border-radius:8px;padding:6px 12px;font-weight:700;font-size:13.5px;">${label}: ${value.toFixed(2)}g</div>`;
  const chips = [];
  if (haaliWt > 0) chips.push(chip("Haali weight", haaliWt, "#B8860B"));
  if (paatWt > 0) chips.push(chip("Paat weight", paatWt, "#0B2A5B"));
  if (silverTotal > 0) chips.push(chip("Total silver weight", silverTotal, "#64748B"));
  if (goldWt > 0) chips.push(chip("Total gold weight", goldWt, "#D4A017"));
  document.getElementById("ornamentWeightSummary").innerHTML = chips.join("");
...
function openValuationLimitModal() {
  document.getElementById("editValuationLimit").value = loanData.valuationLimit || "";
  openModal("valuationLimitModal");
}

async function saveValuationLimit() {
  const value = parseFloat(document.getElementById("editValuationLimit").value) || null;
  await loanRef.update({ valuationLimit: value });
  await logActivity({ customerId: loanData.customerId, loanId, eventType: "loan_edited", action: "Valuation limit updated", detail: value ? fmtMoney(value) : "removed" });
  closeModal("valuationLimitModal");
  toast("Valuation limit updated");
  await loadAll();
}

function proceedPastDisbValuationWarning() {
  closeModal("disbValuationWarningModal");
  disbValuationWarningConfirmed = true;
  document.getElementById("disbForm").requestSubmit();
}

let disbValuationWarningConfirmed = false;
...
function openDisbModal() {
  ...
  disbValuationWarningConfirmed = false;
  openModal("disbModal");
}
...
async function saveDisbursement(e) {
  e.preventDefault();
  const newAmount = parseFloat(document.getElementById("mDisbAmount").value);

  // Soft warning against over-lending relative to the pledged item's
  // value — checks the CUMULATIVE total across every disbursement on this
  // loan (existing ones plus this new one), not just this single amount
  // in isolation, since the concern is the total money out against one
  // valuation, however many disbursements it took to get there. Applies
  // equally to a plain top-up and to a reactivation, since both go
  // through this same function.
  if (loanData.valuationLimit) {
    const existingTotal = disbursements.reduce((s, d) => s + (Number(d.amount) || 0), 0);
    const cumulativeAfter = existingTotal + newAmount;
    if (cumulativeAfter > loanData.valuationLimit && !disbValuationWarningConfirmed) {
      document.getElementById("disbValuationWarningText").textContent =
        `Total disbursed on this loan would become ${fmtMoney(cumulativeAfter)} (${fmtMoney(existingTotal)} already given + ${fmtMoney(newAmount)} now) — ${fmtMoney(cumulativeAfter - loanData.valuationLimit)} above the valuation limit of ${fmtMoney(loanData.valuationLimit)}.`;
      openModal("disbValuationWarningModal");
      return;
    }
  }
  disbValuationWarningConfirmed = false; // reset for next time
