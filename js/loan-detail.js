...
function renderOrnaments() {
  document.getElementById("ornamentBody").innerHTML = ornaments.map((o) => `
    <tr>
      ...
      <td>
        <span style="cursor:pointer;" onclick="openIdentityModal('${o.id}')" title="Click to edit">
          ${o.identityRemark ? escapeHtml(o.identityRemark) : `<span style="color:var(--warn);">+ add note</span>`}
        </span>
      </td>
      <td>${o.released
        ? `<span class="badge badge-closed">Released ${fmtDate(o.releasedAt)}</span>`
        : (loanData.status === "active" ? `<button class="btn btn-secondary btn-sm" onclick="openItemReleaseModal('${o.id}', '${escapeHtml(o.itemName).replace(/'/g, "\\'")}')">Release item</button>` : "Pledged")}
      </td>
...
async function saveDisbursement(e) {
  ...
  const photoFile = document.getElementById("mDisbPhoto").files[0];
  if (photoFile) {
...
// ---------- Identity note per ornament (which bag/pack it's in) ----------
function openIdentityModal(ornId) {
  const o = ornaments.find((x) => x.id === ornId);
  document.getElementById("identityOrnId").value = ornId;
  document.getElementById("identityInput").value = o ? (o.identityRemark || "") : "";
  openModal("identityModal");
}

async function saveIdentityRemark() {
  const ornId = document.getElementById("identityOrnId").value;
  const remark = document.getElementById("identityInput").value.trim();
  await loanRef.collection("ornaments").doc(ornId).update({ identityRemark: remark });
