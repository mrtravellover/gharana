...
async function loadProfile() {
  ...
  loans.sort((a, b) => toJsDate(b.date) - toJsDate(a.date));

  const body = document.getElementById("loanBody");
  if (loans.length === 0) {
    body.innerHTML = `<tr><td colspan="4" style="color:var(--ink-soft);padding:20px 0;">No loans yet for this customer.</td></tr>`;
    return;
  }
