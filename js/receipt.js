...
//
// Five receipt types: Loan, Interest, Renewal, Payment, Item Return.
...
const RECEIPT_TYPES = {
  ...
  gold_return: { label: "Item Return Receipt", prefix: "GR" },
};
...
function buildLoanReceiptBody(ctx) {
  ...
  const closedBanner = loan.status === "closed" ? `
    <div style="background:#E8F5EE;border:1px solid #10B981;border-radius:8px;padding:12px 14px;margin:10px 0;">
      <p style="color:#0A7B4F;font-weight:700;font-size:14px;margin-bottom:4px;">✓ This loan is fully closed — mortgage handed back to customer</p>
      <p style="font-size:12.5px;color:#333;">Handed back on: <strong>${fmtDate(loan.closedAt)}</strong></p>
  ...
  // up-to-date state — not just what was originally disbursed). Uses a
  // small tolerance when comparing to the original amount — without it,
  // ordinary floating-point rounding (e.g. an internal 99999.999999...
  // that displays as a clean ₹1,00,000) could falsely show "Partially
  // paid" on a disbursement nothing has actually been paid toward.
  ...
  const disbRows = disbursements.map((d) => {
    ...
    if (remaining <= 0.01) { statusLabel = "Closed"; statusColor = "#0A7B4F"; }
    else if (remaining < d.amount - 0.01) { statusLabel = "Partially paid"; statusColor = "#B8860B"; }
  ...
  // Which disbursement(s) each payment actually applied to — from the
  // engine's payment-by-payment breakdown, matched to each stored payment
  // record by date+amount (the same matching approach already used for
  // the advance-interest split above).
  const breakdownForRows = summary.paymentBreakdown || [];
  const disbLabel = (id) => {
  ...
  const paymentRows = (payments || [])
    ...
    .map((p) => {
      const bd = breakdownForRows.find((b) => Math.abs(toJsDate(b.date) - toJsDate(p.date)) < 1000 && b.amount === Number(p.amount));
      const touched = bd && bd.touchedDisbursementIds && bd.touchedDisbursementIds.length
        ? bd.touchedDisbursementIds.map(disbLabel).join(", ")
        : "—";
      return `<tr><td>${fmtDate(p.date)}</td><td style="text-transform:capitalize;">${p.type === "advance_interest" ? "Advance interest" : p.type}</td><td>${fmtMoney(p.amount)}</td><td>${touched}</td></tr>`;
    })
    .join("");

  // Which date the still-outstanding principal has actually been counting
  // interest from — the LIVE effective date (reset by the most recent
  // real settlement on that disbursement), not the original disbursement
  // date. If a payment has since cleared interest up to some later date,
  // saying "due from" the original date would be misleading — interest
  // isn't quietly building since day one, it restarts fresh from wherever
  // it was last cleared.
  const outstandingDates = disbursements
    .filter((d) => {
      const live = perDisb.find((p) => p.id === d.id);
      return (live ? live.principal : d.amount) > 0.01;
    })
    .map((d) => {
      const live = perDisb.find((p) => p.id === d.id);
      return fmtDate(live ? live.effectiveDate : d.date);
    });
  ...
  return `
    ...
    ${paymentRows ? `
    <table>
      <thead><tr><th>Payment date</th><th>Type</th><th>Amount</th><th>Disbursement</th></tr></thead>
...
function buildPaymentReceiptBody(ctx) {
  const { loan, payment, summary } = ctx;
  if (payment.isAdvanceInterest) {
    return `
      <p>Customer: <strong>${escapeHtml(loan.customerName)}</strong></p>
      <p>Loan #: <strong>${escapeHtml(loan.loanNumber)}</strong> — advance interest payment</p>
      <table>
        <thead><tr><th>Date</th><th>Amount deposited</th><th>→ Interest settled</th><th>→ Advance credit</th></tr></thead>
        <tbody><tr><td>${fmtDate(payment.date)}</td><td>${fmtMoney(payment.amount)}</td><td>${fmtMoney(payment.settledPortion)}</td><td>${fmtMoney(payment.creditPortion)}</td></tr></tbody>
      </table>
      <p class="receipt-total">Amount received: ${fmtMoney(payment.amount)}</p>
      <p style="font-size:12.5px;color:#555;">Of this, ${fmtMoney(payment.settledPortion)} settled interest already due, and ${fmtMoney(payment.creditPortion)} is credited toward interest not yet accrued.</p>
      ${summary ? `<p class="receipt-total" style="margin-top:8px;">Principal remaining: ${fmtMoney(summary.principalOutstanding)}</p>` : ""}`;
  }
  return `
    <p>Customer: <strong>${escapeHtml(loan.customerName)}</strong></p>
...
function buildInterestReceiptBody(ctx) {
  ...
  if (payment.isAdvanceInterest) {
    return `
      ...
      <p>Loan #: <strong>${escapeHtml(loan.loanNumber)}</strong> — advance interest payment</p>
      <table>
        <thead><tr><th>Date</th><th>Interest settled now</th><th>Advance credit (future interest)</th></tr></thead>
        <tbody><tr><td>${fmtDate(payment.date)}</td><td>${fmtMoney(payment.settledPortion)}</td><td>${fmtMoney(payment.creditPortion)}</td></tr></tbody>
      </table>
      <p class="receipt-total">Total deposited: ${fmtMoney(payment.amount)}</p>
      ${summary ? `<p class="receipt-total">Principal remaining: ${fmtMoney(summary.principalOutstanding)}</p>` : ""}`;
  }
  return `
    <p>Customer: <strong>${escapeHtml(loan.customerName)}</strong></p>
    <p>Loan #: <strong>${escapeHtml(loan.loanNumber)}</strong></p>
...
function buildGoldReturnReceiptBody(ctx) {
  ...
  return `
    ...
    <p class="receipt-total">Handed back to customer on: ${fmtDate(ornament.releasedAt)}</p>
    ${ornament.releaseRemark ? `<p style="font-size:12.5px;color:#555;">Remarks: ${escapeHtml(ornament.releaseRemark)}</p>` : ""}
    ${loan.returnPhotoUrl ? `<img src="${loan.returnPhotoUrl}" width="160" height="160" style="margin-top:10px;border-radius:6px;object-fit:cover;" alt="Return photo">` : ""}
    <p style="font-size:12.5px;color:#555;margin-top:8px;">Please verify the item(s) above before leaving the shop.</p>`;
}
