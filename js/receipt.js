...
// out pages.
//
// SHOP_INFO below is a placeholder — customize with your actual shop
// name, address, and phone. (A proper "Shop Settings" page was
// flagged as a future TODO in an earlier session and would be the
// right place to make this editable in the UI instead of in code.)
// ============================================================

const SHOP_INFO = {
  name: "Shah Jewellers",
  tagline: "Gold & Silver Mortgage Services",
  address: "31 Nehru Marg, Jhabua, M.P. 457661",
  phone: "", // add your shop's phone number here
};
...
// library (loaded from cdnjs). Encodes a short plain-text summary, not a
// URL — there's nothing to link to since this app has no public pages.
...
function generateReceipt(type, ctx) {
  ...
  const qrText = `${SHOP_INFO.name} | ${RECEIPT_TYPES[type].label} | ${receiptNumber} | ${ctx.loan.loanNumber} | ${fmtDate(date)}`;
  renderReceiptQr(qrId, qrText);
