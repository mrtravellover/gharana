...
(function () {
  ...
  async function renderSingleLoanCard(loan) {
    try {
      ...
      addCard(`
        ...
        <div class="owl-w-card-body">
          ...
          <div class="owl-w-row" style="margin-top:6px;"><span class="l">Total payable today</span><span class="v owl-w-v-emerald">${fmtMoney(summary.totalPayableToday)}</span></div>
        </div>
      `);
      setOwlState("happy");
    } catch (err) {
  ...
  async function handleAccountQuery(rawName, receiptNumber) {
    ...
    if (receiptNumber) {
      try {
        ...
        if (!byNumber.empty) {
          ...
          await renderSingleLoanCard({ id: loanDoc.id, ...loanDoc.data() });
          return;
        }
      } catch (err) {
    ...
    try {
      ...
      const depositRowsHtml = activeDeposits
        .map((d) => {
          const interest = d.interestBearing && d.interestRate > 0 ? periodInterest(d.remaining, d.interestRate, d.depositedDate, new Date(), "simple") : 0;
          ...
          return `<div class="owl-w-loan-item">
            ...
            <div class="owl-w-row"><span class="l">Remaining balance</span><span class="v">${fmtMoney(d.remaining)}</span></div>
      ...
      addCard(`
        <div class="owl-w-card-head"><span class="t"><a class="owl-w-link" href="customer-profile.html?id=${match.id}">${escapeHtml(match.name)}</a></span><span class="s">${loanDocs.length} open loan${loanDocs.length === 1 ? "" : "s"}${activeDeposits.length ? ` · ${activeDeposits.length} surplus deposit${activeDeposits.length === 1 ? "" : "s"}` : ""}</span></div>
        <div class="owl-w-card-body">
          ...
          ${depositRowsHtml}
          <div class="owl-w-row"><span class="l">Payable to customer if withdrawn today</span><span class="v owl-w-v-emerald">${fmtMoney(round2(totalSurplusPayable))}</span></div>` : ""}
        </div>
      `);
      setOwlState("happy");
    } catch (err) {
  ...
  function botReply(text) {
    ...
    setTimeout(() => {
      ...
      if (result.action === "account_lookup") {
        handleAccountQuery(result.accountQuery.name, result.accountQuery.receiptNumber);
  ...
  function initOwlAssistant() {
    ...
    addBotText('Hoo-hoo! 🦉 Ask me to <b>check a customer\'s account</b>, or just tell me the numbers in any order — <b>"10000 2.5 24-07-2025"</b>, or even split across a few messages if you don\'t have it all handy. Add <b>"CA"</b> for compounding annually. Hindi/Gujarati/Hinglish work too — try <b>"50000 ka byaaj"</b>.');
  }
