...
function proceedPastValuationWarning() {
  closeModal("valuationWarningModal");
  valuationWarningConfirmed = true;
  document.getElementById("loanForm").requestSubmit();
}

let valuationWarningConfirmed = false;
...
async function saveLoan(e) {
  ...
  const valuationLimit = parseFloat(document.getElementById("valuationLimit").value) || null;

  // Soft warning, not a hard block — a heads-up against over-lending
  // relative to the pledged item's value, that can still be overridden.
  if (valuationLimit && disbAmount > valuationLimit && !valuationWarningConfirmed) {
    document.getElementById("valuationWarningText").textContent =
      `This disbursement (${fmtMoney(disbAmount)}) is ${fmtMoney(disbAmount - valuationLimit)} above the valuation limit you set (${fmtMoney(valuationLimit)}).`;
    openModal("valuationWarningModal");
    return;
  }
  valuationWarningConfirmed = false; // reset for next time
  ...
  const loanRef = await db.collection("loans").add({
    ...
    valuationLimit: valuationLimit || null,
