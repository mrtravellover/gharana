...
requireAuth(async () => {
  ...
  document.getElementById("periodPicker").addEventListener("change", () => { renderEntries(); updateExportPeriodLabel(); renderVillageAnalytics(); });
  document.getElementById("exportCurrentPeriodOnly").addEventListener("change", updateExportPeriodLabel);

  try {
    await loadCustomerVillageMap();
    await ensureWindowCovers(requiredWindowStart(mode));
    buildPeriods();
  } catch (err) {
...
// One cheap fetch of just the customers collection (no subcollections) —
// used to look up which village each loan's customer belongs to, without
// needing a separate read per loan.
async function loadCustomerVillageMap() {
  const snap = await db.collection("customers").get();
  snap.docs.forEach((d) => { const c = d.data(); customerVillageMap[d.id] = c.village || c.address || ""; });
}
...
let processedLoansCache = []; // loan-level records (for Village Analytics) — built as a free byproduct of the fetch above, not a separate fetch
let customerVillageMap = {}; // customerId -> village, fetched once (cheap — one collection, no subcollections)
...
async function loadDataForWindow(windowStart) {
  ...
  await Promise.all(newLoanIds.map(async (loanId) => {
    ...
    const [loanDoc, disbSnap, paySnap, ornSnap] = await Promise.all([
      loanRef.get(),
    ...
    processedLoansCache.push({
      id: loanId,
      date: loan.date,
      status: loan.status,
      customerId: loan.customerId,
      customerName: loan.customerName,
      village: (customerVillageMap[loan.customerId] || "").trim() || "(no village recorded)",
      totalDisbursed: disbursements.reduce((s, d) => s + (Number(d.amount) || 0), 0),
...
// ---------- Village Analytics ----------
// Reuses the exact same loans Reports already loaded for whatever period is
// currently selected (processedLoansCache, built as a byproduct of
// loadDataForWindow above) — no separate fetch, so this adds zero extra
// Firestore reads beyond what Reports needed anyway.
//
// Village grouping is a plain exact match on the customer's address field
// (no normalization) — "Jhabua" and "jhabua" currently count separately.
//
// Interest earned / Recovery % use each matching loan's FULL life-of-loan
// payment history, not just payments within the selected period — so this
// reads as "total interest collected so far, on loans that started in this
// period," not "interest collected during this period" on loans generally.
let vaExpandedVillage = null;

function currentPeriodLoans() {
  const k = document.getElementById("periodPicker").value;
  if (!k) return [];
...
function renderVillageAnalytics() {
  const loans = currentPeriodLoans();
  const k = document.getElementById("periodPicker").value;
  document.getElementById("villagePeriodLabel").textContent = k ? PERIOD_CONFIG[mode].labelFn(k) : "no period selected";
  document.getElementById("villageCount").textContent = "";

  const byVillage = {};
  loans.forEach((l) => {
    const key = normalizeVillageKey(l.village);
    if (!byVillage[key]) byVillage[key] = { village: l.village, variantCounts: {}, loans: [], customerIds: new Set() };
    byVillage[key].variantCounts[l.village] = (byVillage[key].variantCounts[l.village] || 0) + 1;
    byVillage[key].loans.push(l);
    byVillage[key].customerIds.add(l.customerId);
  });
  // Use whichever exact casing appeared most often within the group as the
  // display label — "Jhabua" and "jhabua" merge into one row, but the row
  // shows real, human-written text rather than a forced-lowercase version.
  Object.values(byVillage).forEach((v) => {
    v.village = Object.entries(v.variantCounts).sort((a, b) => b[1] - a[1])[0][0];
  });

  const villages = Object.values(byVillage).map((v) => {
    const active = v.loans.filter((l) => l.status === "active").length;
    const closed = v.loans.filter((l) => l.status === "closed").length;
    const totalDisbursed = v.loans.reduce((s, l) => s + l.totalDisbursed, 0);
    const totalCollected = v.loans.reduce((s, l) => s + l.totalCollected, 0);
    const totalInterest = v.loans.reduce((s, l) => s + l.interestCollected, 0);
    const totalWeight = v.loans.reduce((s, l) => s + l.totalWeight, 0);
    return {
      village: v.village,
      customers: v.customerIds.size,
      active, closed,
      interestEarned: round2(totalInterest),
      avgLoan: v.loans.length ? round2(totalDisbursed / v.loans.length) : 0,
      avgWeight: v.loans.length ? round2(totalWeight / v.loans.length) : 0,
      recoveryPct: totalDisbursed > 0 ? round2((totalCollected / totalDisbursed) * 100) : 0,
      totalDisbursed,
      loans: v.loans,
    };
  }).sort((a, b) => b.totalDisbursed - a.totalDisbursed);

  document.getElementById("villageCount").textContent = villages.length ? `(${villages.length})` : "";
  renderVillageChart(villages);
  renderVillageTable(villages);
}

function renderVillageChart(villages) {
  const container = document.getElementById("villageChart");
  if (!villages.length) { container.innerHTML = `<p style="color:var(--ink-soft);">No loans in this period.</p>`; return; }

  const top = villages.slice(0, 10);
  const maxVal = Math.max(1, ...top.map((v) => v.totalDisbursed));
  container.innerHTML = top.map((v) => {
    const pct = Math.max(2, Math.round((v.totalDisbursed / maxVal) * 100));
    return `
    <div class="bar-row" onclick="toggleVillageExpand('${v.village.replace(/'/g, "\\'")}')">
      <div class="bar-label" title="${escapeHtml(v.village)}">${escapeHtml(v.village)}</div>
      <div class="bar-track"><div class="bar-fill" style="width:${pct}%;"></div></div>
      <div class="bar-value mono">${fmtMoney(v.totalDisbursed)}</div>
    </div>`;
  }).join("");
}

function toggleVillageExpand(village) {
  vaExpandedVillage = vaExpandedVillage === village ? null : village;
  renderVillageAnalytics();
}

function renderVillageTable(villages) {
  const body = document.getElementById("villageBody");
  if (!villages.length) {
    body.innerHTML = `<tr><td colspan="8" style="color:var(--ink-soft);padding:20px 0;">No loans in this period.</td></tr>`;
    return;
  }

  body.innerHTML = villages.map((v) => {
    const rowHtml = `
    <tr class="village-row" onclick="toggleVillageExpand('${v.village.replace(/'/g, "\\'")}')">
      <td>${escapeHtml(v.village)}</td>
      <td>${v.customers}</td>
      <td>${v.active}</td>
      <td>${v.closed}</td>
      <td class="mono">${fmtMoney(v.interestEarned)}</td>
      <td class="mono">${fmtMoney(v.avgLoan)}</td>
      <td class="mono">${v.avgWeight.toFixed(2)}</td>
      <td class="mono">${v.recoveryPct}%</td>
    </tr>`;

    if (vaExpandedVillage !== v.village) return rowHtml;

    const customerRows = v.loans
      .filter((l, i, arr) => arr.findIndex((x) => x.customerId === l.customerId) === i)
      .map((l) => `<a href="customer-profile.html?id=${l.customerId}" class="btn btn-ghost btn-sm" style="margin:2px;">${escapeHtml(l.customerName)}</a>`)
      .join("");

    return rowHtml + `
    <tr class="village-customers-row">
      <td colspan="8">
        <strong style="font-size:13px;">Customers in ${escapeHtml(v.village)}:</strong><br>
        ${customerRows}
      </td>
    </tr>`;
  }).join("");
