...
// Surplus funds a customer has asked back — pulled via a collection-group
// query across every customer's deposits subcollection at once, so this
// doesn't need to loop through customers one at a time.
async function renderFundsToReturn() {
  ...
  let snap;
  try {
    snap = await db.collectionGroup("deposits").where("status", "==", "requested").get();
  } catch (err) {
  ...
  deposits.sort((a, b) => toJsDate(a.requestedDate) - toJsDate(b.requestedDate));

  list.innerHTML = deposits.map((d) => {
    const interest = d.interestBearing && d.interestRate > 0 ? periodInterest(d.amount, d.interestRate, d.depositedDate, new Date(), "simple") : 0;
    const payable = round2(d.amount + interest);
    return `
    <div class="disb-card" style="cursor:pointer;" onclick="location.href='customer-profile.html?id=${d.customerId}'">
      <div class="row"><strong>${escapeHtml(d.customerName || "Customer")}</strong><span class="badge" style="background:var(--warn-soft);color:var(--warn);">requested</span></div>
      <div class="row"><span class="k">Deposited</span><span>${fmtDate(d.depositedDate)}</span></div>
      <div class="row"><span class="k" style="color:var(--good);">Payable</span><strong class="mono" style="color:var(--good);">${fmtMoney(payable)}</strong></div>
    </div>`;
  }).join("");
}
