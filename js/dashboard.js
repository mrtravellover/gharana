...
async function renderGreeting() {
  ...
  try {
    ...
    if (doc.exists && doc.data().displayName) name = doc.data().displayName.split(" ")[0]; // first name only, for the greeting
  } catch (err) {
    console.error("Couldn't load profile for greeting:", err);
  }

  if (!name) {
    // Fallback for anyone who hasn't set a profile name yet — same as before.
    const email = (auth.currentUser && auth.currentUser.email) || "";
    const namePart = email.split("@")[0].replace(/[._]/g, " ");
    name = namePart ? namePart.replace(/\b\w/g, (c) => c.toUpperCase()) : "";
  }
...
async function loadDashboard() {
  ...
  const overdueLoans = []; // active loans with no payment in 90+ days

  // Fetch every loan's disbursements + payments IN PARALLEL, not one loan at a time —
  // sequential fetches here is the single biggest cause of a slow-loading dashboard.
  await Promise.all(loansSnap.docs.map(async (doc) => {
    ...
    // Same definition as the "Overdue" filter on the Loans page — active
    // loan, no payment in the last 90 days (falls back to loan date if
    // there's never been a payment at all).
    const lastPaymentDate = payments.length ? payments.map((p) => toJsDate(p.date)).sort((a, b) => b - a)[0] : null;
    const daysSincePayment = daysBetween(lastPaymentDate || loan.date, new Date());
    if (daysSincePayment >= 90) {
      overdueLoans.push({ id: loan.id, loanNumber: loan.loanNumber, customerName: loan.customerName, daysSincePayment, totalPayableToday: summary.totalPayableToday });
    }
  }));
...
// Specific withdrawal AMOUNTS a customer needs brought to them (not whole
// deposits — since a deposit can be partially withdrawn multiple times).
// Pulled via a collection-group query across every customer/deposit's
// withdrawals subcollection at once, so this doesn't need to loop through
// customers one at a time.
async function renderFundsToReturn() {
  const card = document.getElementById("fundsToReturnCard");
  const list = document.getElementById("fundsToReturnList");
  const countEl = document.getElementById("fundsToReturnCount");

  let snap;
  try {
    snap = await db.collectionGroup("withdrawals").where("status", "==", "pending").get();
  } catch (err) {
    console.error("Couldn't load funds-to-return:", err);
  ...
  withdrawals.sort((a, b) => toJsDate(a.date) - toJsDate(b.date));

  list.innerHTML = withdrawals.map((w) => `
    <div class="disb-card" style="cursor:pointer;" onclick="location.href='customer-profile.html?id=${w.customerId}'">
      <div class="row"><strong>${escapeHtml(w.customerName || "Customer")}</strong><span class="badge" style="background:var(--warn-soft);color:var(--warn);">bring it</span></div>
      <div class="row"><span class="k">Needed for</span><span>${fmtDate(w.date)}</span></div>
      <div class="row"><span class="k" style="color:var(--good);">Amount</span><strong class="mono" style="color:var(--good);">${fmtMoney(w.amount)}</strong></div>
      ${w.notes ? `<div class="row"><span class="k">Notes</span><span>${escapeHtml(w.notes)}</span></div>` : ""}
    </div>`).join("");
}
...
function renderOverdue(loans) {
  ...
  loans.sort((a, b) => b.daysSincePayment - a.daysSincePayment); // most overdue first
  list.innerHTML = loans.slice(0, 10).map((l) => `
    <div class="disb-card" style="cursor:pointer;" onclick="location.href='loan-detail.html?id=${l.id}'">
      <div class="row"><strong>${escapeHtml(l.loanNumber)} — ${escapeHtml(l.customerName)}</strong><span class="badge" style="background:var(--danger-soft);color:var(--danger);">${l.daysSincePayment} days</span></div>
      <div class="row"><span class="k" style="color:var(--danger);">Total payable</span><strong class="mono" style="color:var(--danger);">${fmtMoney(l.totalPayableToday)}</strong></div>
    </div>`).join("");
  if (loans.length > 10) {
    list.innerHTML += `<a href="loans.html" class="btn btn-ghost btn-sm" style="margin-top:8px;">+${loans.length - 10} more — view all on Loans</a>`;
  }
}

// Nudges toward Backup only after it's been genuinely a while (90+ days,
// or never done at all) — not on every visit, which would just become
// background noise you learn to ignore.
async function renderBackupNudge() {
  const card = document.getElementById("backupNudgeCard");
  const text = document.getElementById("backupNudgeText");
  try {
    const doc = await db.collection("settings").doc("backup").get();
    const lastBackupAt = doc.exists ? doc.data().lastBackupAt : null;
    if (!lastBackupAt) {
      card.style.display = "";
      text.textContent = "You haven't taken a backup yet. Worth doing once, then every few months.";
      return;
    }
    const daysSince = daysBetween(lastBackupAt, new Date());
    if (daysSince >= 90) {
      card.style.display = "";
      text.textContent = `It's been ${daysSince} days since your last backup (${fmtDate(lastBackupAt)}). Worth taking a fresh one.`;
    } else {
      card.style.display = "none";
    }
  } catch (err) {
    console.error("Couldn't check last backup date:", err);
    card.style.display = "none";
  }
}
