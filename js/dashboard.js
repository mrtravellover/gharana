...
async function loadDashboard() {
  ...
  renderReadyToRelease(await Promise.all(releasedSnap.docs.map(async (d) => {
    ...
    const ornSnap = await db.collection("loans").doc(loan.id).collection("ornaments").get();
...
function renderReadyToRelease(loans) {
  ...
  list.innerHTML = loans.map((l) => {
    ...
    <div class="disb-card" style="cursor:pointer;" onclick="location.href='loan-detail.html?id=${l.id}'">
      ...
      ${pending.map((o) => `
        <div class="row"><span class="k">${escapeHtml(o.itemName)}</span><span>${o.identityRemark ? escapeHtml(o.identityRemark) : `<span style="color:var(--warn);">no note saved</span>`}</span></div>
      `).join("") || `<div class="row"><span class="k" style="color:var(--warn);">No items / notes found</span></div>`}
    </div>`;
  }).join("");
}
