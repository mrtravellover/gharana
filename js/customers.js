...
function closeCustomerModal() { document.getElementById("customerModal").style.display = "none"; document.getElementById("customerForm").reset(); }
...
async function saveCustomer(e) {
  ...
  const photoFile = document.getElementById("custPhoto").files[0];
  if (photoFile) {
    const url = await uploadPhoto(photoFile, `customer-photos/${ref.id}-${Date.now()}.jpg`);
